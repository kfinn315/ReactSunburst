...
export function SunburstContainer({ dimensions, rootNode, getHighlighter, onMouseEnter, onMouseLeave, colorScale, centerColor, duration, minWidth = 20 }: SunburstContainerProps) {
    ...
    const nodes = partitionTreeLayout<SunburstItem>(rootNode, sunburstDimensions).descendants()
    ...
    const mouseEnterHandler: SunburstEvent<SunburstItemTreeNode> = (event: MouseEvent, d: HierarchyNode<SunburstItemTreeNode>): void => {
        setDetail(itemDetail(d));
    ...
    return <div className="visualization-wrapper">
        <div className='sunburst-wrapper'>
            <svg width={svgDimension} height={svgDimension} viewBox={`0 0 ${String(svgDimension)} ${String(svgDimension)}`}>
                <Sunburst<SunburstItem> getHighlighter={getHighlighter} getArcColor={getArcColor} radius={radius} items={nodes} onMouseEnter={mouseEnterHandler} onMouseLeave={mouseLeaveHandler} isArcClickable={() => false} duration={duration} />
            </svg>
        </div>
