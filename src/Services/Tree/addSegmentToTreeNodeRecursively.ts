...
export function addSegmentToTreeNodeRecursively<TNode>({ idGenerator, treeNode, segmentIterator, data }: Props<TNode>) {
    ...
    if (nextSegment.done) { //end of segments, set segment.data to node.data and end recursion by returningF
        treeNode.data = data;
    } else {
        ...
        if (childNode === undefined) { //create new node and add to children
        ...
        addSegmentToTreeNodeRecursively({ idGenerator, treeNode: childNode, segmentIterator, data })
    }
}
